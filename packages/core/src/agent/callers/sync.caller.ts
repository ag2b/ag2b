 ...
 * When a sink is provided, fabricates `agent_*` reasoning/content/tool-call events from
 * the single response so consumers of `chat({ onEvent })` see the same event sequence
 * as the non-streamable fallback in {@link createStreamCaller}.
 * @param provider - The LLM provider to call.
 * @param sink - Optional sink to receive fabricated reasoning/content/tool-call events.
 */
